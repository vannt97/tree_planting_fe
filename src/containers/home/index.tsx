...
import bgAboutProgramMobile from 'public/images/bg-about-program-mobile.png';
...
import productHome1 from 'public/images/product-home-1.jpg';
import productHome2 from 'public/images/product-home-2.png';
import productHome3 from 'public/images/product-home-3.jpg';
...
function HomePage({ imageData: dataImage }) {
  ...
  useEffect(() => {
    ...
    if (router.asPath === '/#dang-ky-bao-hanh') {
      const header = document.getElementById('header');
      const titleElement = document.getElementById('regiterWarranty');
      window.scrollTo({
        top: titleElement.getBoundingClientRect().top - (header.clientHeight + 60) + window.scrollY,
        left: 0,
        behavior: 'smooth',
      });
    }
  ...
  const SilderPartnerMobile = useMemo(() => {
    let colNumber = 4;
    let rowNumber = Math.ceil((listImgPartners as [])?.length / colNumber);
    let array2D = conver1DArrayTo2DArray(listImgPartners, rowNumber, colNumber);

    return array2D?.map((partners, index) => {
      return (
        <SwiperSlide key={index}>
          <div className="inline-grid grid-cols-2 gap-8">
            {partners.map((item, indexI) => (
              <div key={indexI} className="flex items-center">
                {/* <img width={150} height={150} src={item.url} alt /> */}
                <Image width={150} height={150} src={item.url} alt="" layout="fixed" />
              </div>
            ))}
          </div>
        </SwiperSlide>
  ...
  const handleRedirectToElement = (id: string) => {
    const header = document.getElementById('header');
    const titleElement = document.getElementById(id);
    // console.log('getBoundingClientRect title id: ', titleElement.getBoundingClientRect());
    // // console.log('top title id: ', titleElement.getBoundingClientRect().top);
    // console.log(
    //   'id: ',
    //   titleElement.getBoundingClientRect().top - (header.clientHeight + 0) + window.scrollY
    // );
    window.scrollTo({
      top: titleElement.getBoundingClientRect().top - (header.clientHeight + 0) + window.scrollY,
      left: 0,
      behavior: 'smooth',
    });
  };
      ...
      <section className="home z-0">
        ...
        <article className="home__tutorial container-custom relative z-[5]">
          <img
            src={decorLeaf1.src}
            alt="leaf"
            className="hidden tablet:block absolute w-[12%] -top-[4%] left-0"
          />
          <img
            src={decorLeaf2.src}
            alt="leaf"
            className="hidden tablet:block absolute  -bottom-[40%] right-0"
          />

          <h1 className="color-primary font-bold uppercase text-center mb-3 mt-8 heading-1">
            CÁCH THỨC THAM GIA&nbsp;
          ...
          <p className=" text-center text-black-600 mb-6 ">
            <span className="inline mobileSM:block">
              Rừng đầy ắp những câu chuyện truyền cảm hứng về cuộc sống khỏe mạnh.
            </span>{' '}
            <span className="inline mobileSM:block">
              Nếu rừng không còn cây, chuyện hay cũng không còn.
            </span>{' '}
            <span className="inline mobileSM:block">
              Hãy cùng Panasonic sống khỏe, góp xanh để giữ chuyện rừng còn mãi.
            </span>
          </p>

          <p className=" text-center text-black-600 block ">
            <span className="inline mobileSM:block">
              Từ ngày <span className="text-green-primary font-bold">1/11/2023 - 31/3/2024</span>{' '}
            </span>{' '}
            <span className="inline mobileSM:block">
              với mỗi sản phẩm thuộc bộ{' '}
              <span className="font-bold text-green-primary">GIẢI PHÁP SỨC KHỎE TOÀN DIỆN</span>{' '}
              được bán ra,
            </span>{' '}
            <span className="block mobileSM:inline">
              Panasonic Việt Nam sẽ trồng một cây xanh để phát triển rừng.
            </span>
          </p>

          <div className="w-[95%] tablet:w-[100%] desktop:w-[90%] mx-auto grid grid-cols-1 mobileSM:grid-cols-2 tablet:grid-cols-3 my-10 gap-8 tablet:gap-4 tablet:gap-8">
            <div className="home__intro-wrapper">
              <a
                onClick={() => {
                  handleRedirectToElement('products-wrapper');
                }}
                className="bg-secondary-color home__intro"
              >
                ...
                <span className="color-primary home__intro-text  mt-2 text-center font-bold">
                  Mỗi sản phẩm - Một cây xanh
                </span>
              </a>
              <p className="text-center ">
                Khi mua mỗi sản phẩm{' '}
                <span className="block">
                  trong bộ <span className="font-bold">GIẢI PHÁP SỨC KHỎE TOÀN DIỆN</span>,
                </span>{' '}
                bạn góp thêm 1 cây xanh mới được trồng
              </p>
            </div>
            ...
            <div className="home__intro-wrapper">
              <a
                onClick={() => {
                  handleRedirectToElement('regiterWarranty');
                }}
                to="regiterWarranty"
                className="bg-secondary-color home__intro "
              >
                ...
                <span className="color-primary mt-2 text-center  font-bold home__intro-text">
                  Kích hoạt bảo hành
                </span>
              </a>
              <p className="text-center">
                Kích hoạt bảo hành trên ứng dụng My Panasonic,
                <span className="block"> nhận mã cây để theo dõi thông tin,</span>
                hành trình trưởng thành của cây
              </p>
            </div>
            ...
            <div className="home__intro-wrapper">
              ...
              <p className="text-center">
                Đã bao lâu bạn chưa ghé thăm cây?{' '}
                <span className="block">Nhập mã số cây, xem nay &quot;em ấy&quot;</span> đã lớn tới
                đâu
              </p>
            </div>
          </div>
        ...
        <article id="products-wrapper" className="bg-secondary-color pt-4 pb-10 relative z-[4]">
          <img
            className="hidden tablet:block absolute -bottom-[28%] left-0 z-1"
            src={decorLeaf3.src}
            alt=""
          />
          <img
            className="hidden tablet:block absolute -bottom-[22%] right-0 z-1"
            src={decorLeaf4.src}
            alt=""
          />
          <div className="container-custom relative z-[5]">
            <h1 className="color-primary font-bold uppercase text-center mb-3 tablet:mb-3 mt-4 heading-1">
              MỖI SẢN PHẨM SỐNG KHỎE <span className="block">GÓP MỘT CÂY XANH CHO RỪNG </span>
            </h1>
            <p className=" text-center w-full tablet:w-[90%] laptop:w-[65%] mx-auto text-black-600">
              <span className="inline tablet:block">
                Tận hưởng cuộc sống khỏe mạnh hơn{' '}
                <span className="block mobileSM:inline">
                  {' '}
                  mỗi ngày với GIẢI PHÁP SỨC KHỎE TOÀN DIỆN
                </span>{' '}
                từ Panasonic Việt Nam.
              </span>{' '}
              <span className="inline tablet:block">
                Khỏe từ bầu không khí trong lành, khỏe từ thực phẩm tươi ngon, khỏe từ dòng nước
                tinh khiết.
              </span>{' '}
              Sống khỏe với Panasonic để cùng góp xanh ngay hôm nay.
            </p>
            <div className="w-[95%] mx-auto mobileSM:w-full grid grid-cols-2 mobileSM:grid-cols-4 my-6 mobileSM:my-10 gap-6 mobileSM:gap-4 tablet:gap-8 ">
              <div
                style={{
                  boxShadow: '0px 4px 12px 0px rgba(0, 0, 0, 0.25)',
                }}
                className="flex justify-center mx-auto w-[100%]"
              >
                <a
                  rel="noopener noreferrer"
                  target="_blank"
                  href="https://www.panasonic.com/vn/events-and-promotions/events/song-khoe-gop-xanh-cung-panasonic.html?utm_source=microsite&utm_medium=banner.AC&utm_campaign=treeplanting2023.10230324#dieu-hoa"
                >
              ...
              >
                <a
                  rel="noopener noreferrer"
                  target="_blank"
                  href="https://www.panasonic.com/vn/events-and-promotions/events/song-khoe-gop-xanh-cung-panasonic.html?utm_source=microsite&utm_medium=banner.REF&utm_campaign=treeplanting2023.10230324#tu-lanh"
                >
              ...
              >
                <a
                  target="_blank"
                  rel="noopener noreferrer"
                  href="https://www.panasonic.com/vn/events-and-promotions/events/song-khoe-gop-xanh-cung-panasonic.html?utm_source=microsite&utm_medium=banner.WM&utm_campaign=treeplanting2023.10230324#may-giat"
                >
              ...
              >
                <a
                  target="_blank"
                  rel="noopener noreferrer"
                  href="https://www.panasonic.com/vn/events-and-promotions/events/song-khoe-gop-xanh-cung-panasonic.html?utm_source=microsite&utm_medium=banner.SDA&utm_campaign=treeplanting2023.10230324#cac-san-pham-gia-dung"
                >
        ...
        <article id="regiterWarranty" className="home__form pt-12 laptop:pb-24 pb-32 z-[3]">
          <img
            className="hidden tablet:block absolute -bottom-[35%] left-0"
            src={decorLeaf5.src}
            alt=""
          />
          <div className="container-custom">
            <div className="flex flex-wrap items-center">
              <div className="w-full laptop:w-[35%] text-center">
                <h2 className="block laptop:hidden mb-0 tablet:mb-4 text-center font-bold text-2xl tablet:text-4xl text-secondary-color">
                  ĐĂNG KÝ BẢO HÀNH ĐIỆN TỬ
                </h2>

                <div className="my-0 flex laptop:block items-center justify-evenly ">
                  <div className=" home__form-image  my-0 tablet:my-4">
              ...
              <div className="w-full laptop:w-[65%]">
                <h1 className="hidden laptop:block mb-4 text-center font-bold heading-1 text-secondary-color">
                  ĐĂNG KÝ BẢO HÀNH ĐIỆN TỬ
                </h1>
                <p className="block laptop:hidden text-center text-white-500 mb-8">
                  Kích hoạt bảo hành để nhận mã số cây và các thông tin chi tiết về hành trình phát
                  triển cây của bạn
                </p>
        ...
        <article
          style={{
            backgroundImage: `url(${bgTheoDoiCayCuaBan.src})`,
          }}
          id="followTree"
          className="h-full mobileSM:h-[190vw] tablet:h-full laptop:h-[80vw] desktop:h-[90vw] bg-cover bg-bottom bg-no-repeat relative z-[2] pt-20 pb-80 mobileSM:pb-20"
        >
          <div className="flex flex-wrap items-center pb-[60px] laptop:pb-[100px] desktop:pb-[200px]">
            <div className="w-full tablet:w-[60%] laptop:w-[65%]">
              <img className="w-[90%] mx-auto" src={mountainHomePage.src} alt="" />
            </div>
            <div className="w-full tablet:w-[40%] laptop:w-[35%] px-4 tablet:px-6">
              <div className="home__follow w-full desktop:w-[80%]">
                <h1 className="color-primary font-bold uppercase text-center mb-6 heading-1">
                  theo dõi cây của bạn
                </h1>
                <p className="text-black-600 mb-8 mobileSM:mb-4 text-center">
                  Nhập ngay số điện thoại và mã số cây để theo dõi quá trình sinh trưởng của cây và
                  các hoạt động hấp dẫn khác.
                </p>
                <HomeFormFollow />
              </div>
            </div>
          ...
          <div className="container-custom">
            <div className="w-full laptop:w-[370px] desktop:w-[550px] translate-x-[0] laptop:translate-x-[30px] laptop:translate-x-[0]">
              <h1 className="text-center tablet:text-left color-primary font-bold uppercase mb-3 heading-1">
                cuộc thi
                <span className="block whitespace-nomarl laptop:whitespace-nowrap">
                  &quot;GÓP CHUYỆN RỪNG HAY -{' '}
                  <span className="block mobileSM:inline">RƯỚC NGAY QUÀ CHẤT&quot;</span>
                </span>
                <span className="font-semibold tablet:font-bold text-[14px] tablet:text-[36px] desktopXXL:text-[42px] block whitespace-nomarl laptop:whitespace-nowrap">
                  Thời gian: 18/11/2023-12/01/2024
                </span>
              </h1>
              <p className="pb-10 mobileSM:pb-6 text-justify">
              ...
              <div className="text-center mobileSM:text-left ">
                <button
                  type="button"
                  className="py-3 px-10 font-bold home__form-btn text-white-500 text-xl tablet:text-2xl"
                  onClick={() => {
                    router.push('/gallery');
                  }}
                >
        ...
        <article
          style={{
            backgroundImage: `url(${width > 640 ? bgAboutProgram.src : bgAboutProgramMobile.src})`,
          }}
          id="aboutProgram"
          className="bg-transparent py-[5rem] mobileSM:py-[4rem] tablet:py-[9rem] laptop:py-[12rem] desktop:py-[18rem] desktopXL:py-[20rem] relative z-[3] bg-center bg-cover bg-no-repeat -translate-y-[50px] -mb-[50px] tablet:-mb-[135px] tablet:-translate-y-[100px] laptop:-mb-[100px]"
        >
          <div className="grid grid-cols-1 laptop:grid-cols-2 items-center mx-auto gap-4 mobileSM:gap-7 w-[90%] desktop:w-[80%] ">
            <div
              style={{
                boxShadow: '0px 4px 12px 0px rgba(0, 0, 0, 0.25)',
              }}
            ...
            >
              <img
                className="w-full mobileSM:w-[75%] mx-auto tablet:w-full"
                src={aboutProgram.src}
                alt=""
              />
            </div>
            <div className="text-white-500 text-justify">
              <h1 className="text-white-500 font-bold text-center tablet:text-left uppercase heading-1">
                về chương trình
              </h1>
              <p className="mb-2">
                Chương trình &quot;Sống khỏe góp xanh - Giữ chuyện rừng còn mãi&quot; năm 2023 tiếp
                nối sự thành công của chương trình &quot;Sống khỏe góp xanh&quot; năm trước nhằm
                hiện thực hóa cam kết Green Impact của Tập đoàn Panasonic:{' '}
              </p>
              ...
              <p className="mb-3">
                ...
                tính minh bạch, quy mô lớn, phạm vi ảnh hưởng bao trùm cả về mặt địa lý và đối tượng
                tham gia:
              </p>
              <ul className="list-disc pl-7 pb-2">
                <li>
                  Năm 2022, chương trình đã được triển khai tại 13 Rừng quốc gia và Khu bảo tồn
                  thiên nhiên ở 13 tỉnh thành.
                </li>
                <li>
                  Năm 2023, chương trình dự kiến sẽ góp xanh thêm 7 khu vực mới để phát triển rừng
                  Việt Nam.
                </li>
                <li>Chương trình trồng rừng đầu tiên có sự chung tay của khách hàng và đối tác.</li>
                <li>
                  Chương trình có sự đồng hành của Trung tâm truyền thông tài nguyên môi trường - Bộ
                  Tài Nguyên Môi trường cùng các đơn vị truyền thông khác.
                </li>
                <li>
                  Cây trồng trong chương trình được lựa chọn với tiêu chí tối ưu hóa lượng CO2 hấp
                  thụ, góp phần phủ xanh các khu vực đồi núi trọc, bảo tồn hệ sinh thái địa phương.
                </li>
              </ul>
            </div>
          </div>
          <img
            className="hidden laptop:block absolute right-0 -top-[15%]"
            src={decorLeaf7.src}
            alt=""
          />

          <img
            className="hidden laptop:block absolute left-0 -top-[8%]"
            src={decorLeaf9.src}
            alt=""
          />
          <img
            className="hidden laptop:block absolute right-0 -bottom-[14%]"
            src={decorLeaf10.src}
            alt=""
          />
        </article>

        <article className="relative -mb-[40px] -translate-y-[40px] laptop:-translate-y-[100px] laptop:-mb-[100px] ">
          <Map></Map>
        </article>
        ...
        <article
          ...
          id="partner"
          className="h-full desktopXL:h-[815px] overflow-hidden home__partner-wrapper pt-[1rem] pb-[3rem] tablet:py-[2rem] laptop:py-[5rem] desktop:py-[7rem] bg-center bg-no-repeat bg-auto laptop:bg-cover home__partner relative laptop:-translate-y-[100px] laptop:-mb-[100px]"
        >
          <div className="container-custom w-full">
            <div className="py-4 laptop:py-10 text-center">
              <h1 className="text-white-500 font-bold uppercase text-center mb-4 laptop:mb-8 heading-1">
                Đối Tác
              </h1>
              ...
              {width < 640 ? (
                ...
                <div className="inline-grid w-[90%] desktopXL:w-[75%]  grid-cols-6  gap-8">
        ...
        <div id="news" className="pt-12 pb-[6rem] laptop:pb-[10rem] bg-gray-100 -mb-[6rem]">
          <HomePost />
        </div>
