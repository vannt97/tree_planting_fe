...
function TreeShareComp(props: TreeShareCompProps) {
    ...
    <>
      {data && data?.length > 0 && (
        <div
          ...
          className={`treeShare pb-4 relative ${w100 ? 'w-full' : ''}`}
        >
          <div className="absolute top-0 right-0 z-[-1] left-0 bottom-0">
            <img src={bgTreeSharing.src} className="w-full h-full" alt="" />
          </div>
          <div className="absolute top-0 h-[7px] w-[28%] left-[50%] translate-x-[-50%] bg-green-primary"></div>
          <div className="p-4">
            <div className="grid grid-cols-3 gap-4 treeShare__header">
              ...
              <div className="flex items-center justify-center">
                <div className="">
                  <h1 className="text-green-primary font-extrabold text-[0.8rem] tablet:text-[0.9rem] laptop:text-[1.6rem]">
                    {data[0]?.treeName}
                  </h1>
                </div>
              </div>
            ...
            <p className="text-center italic color-text-1e text-[0.6rem] laptop:text-[1rem] font-medium">
          ...
          <div className="flex my-4">
            <div className="flex treeShare__body-des items-center">
              <div className="treeShare__body-des-bg py-2 pl-2 laptop:py-4 laptop:pl-4 bg-1e">
                <span className="treeShare__body-leaf">
                  <img height={20} width={30} src={treeShareLeaf.src} alt="" />
                </span>
                <p className="text-white-500 treeShare__body-des-text text-[0.4rem] mobile:text-[0.5rem] tablet:text-[0.6rem] laptop:text-[0.8rem]">
            ...
            <div className="flex items-center treeShare__body-image justify-center">
              <div className="treeShare__body-image-div">
                <span>{<img src={TreeImage.src} alt="" />}</span>
              </div>
            </div>
            ...
            <div className="flex items-start treeShare__body-title pr-4 flex-col justify-center">
              ...
              <p className="font-bold text-[0.45rem] mobile:text-[0.65rem] laptop:text-[1rem] color-text-ee">
                {data[0]?.location}
              </p>
            </div>
          </div>
          <p className="text-center font-bold text-[0.6rem] mobile:text-[0.7rem] laptop:text-[1rem] text-green-primary italic">
            Được trồng bởi
          </p>
          <h1 className="text-center font-black text-[0.6rem] mobile:text-[0.7rem] tablet:text-[0.9rem] laptop:text-[1.34rem]">
